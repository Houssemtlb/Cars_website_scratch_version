...
class VehiculeModel extends Model{
    ...
    {
        $this->connect();
        $request = $this->connection->prepare("INSERT INTO vehicule(`marque_id`, `model`, `version`, `annee`, `prix`, `moteur`, `cylindrée`, `consommation`, `nb_cylindres`, `nb_places`, `type`, `longueur`, `largeur`, `hauteur`, `note`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $request->execute(array($data['marque_id'],$data['model'],$data['version'], $data['annee'], $data['prix'],$data['moteur'],$data['cylindrée'],$data['consommation'],$data['nb_cylindres'],$data['nb_places'],$data['type'],$data['longueur'],$data['largeur'],$data['hauteur'],$data['note']));
        $this->disconnect();
    }
    ...
    {
        $this->connect();
        $request = $this->connection->prepare("UPDATE vehicule SET  marque_id = :marque_id, model = :model, version = :version, annee = :annee, prix = :prix, moteur = :moteur, cylindrée = :cylindrée, consommation = :consommation, nb_cylindres = :nb_cylindres, nb_places = :nb_places, type = :type, longuer = :longueur, largeuur = :largeur, hauteur = :hauteur, note = :note  WHERE (vehicule_id = :vehicule_id)");
        ...
        $request->bindValue(':cylindrée', $data['cylindrée']);
