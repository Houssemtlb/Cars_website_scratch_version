...
class MarqueController extends Controller{
    public function loadPage($id)
    {
        ...
        $menuBar->show(null);
        if(!empty($id[1])){
            $marqueSpecifiqueData = ["marque" => $marques->get($id[1]),"logo" => $images->getMarqueLogo($id[1]) ,"vehicules" => $vehicules->getAllForMarque($id[1]), "images" => $images->getMarqueAllVehiculeImages($id[1])];
